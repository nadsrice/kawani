...
class Ion_auth_acl_model extends Ion_auth_model
{
     ...
     * @return bool
     */
    public function create_permission($perm_key =  FALSE, $perm_name = '', $module_id = '', $function_id = '')
    {
        ...
        $existing_permissions = $this->db->get_where($this->tables['permissions'], array('method' => $perm_key))->num_rows();
    ...
    public function update_permission($permission_id = FALSE, $perm_key = FALSE, $additional_data = array())
    {
        ...
        if (!empty($perm_key))
        {
            ...
            $existing_permission = $this->db->get_where($this->tables['permissions'], array('method' => $perm_key))->row();
        ...
        if($this->config->item('admin_permission', 'ion_auth_acl') === $permission->method && $perm_key !== $permission->method)
        {
    ...
    public function remove_permission($permission_id = FALSE)
    {
        ...
        if($permission->method == $this->config->item('admin_permission', 'ion_auth_acl'))
        {
    ...
    public function permissions($format = 'ids', $key = 'id')
    {
        ...
        foreach($result as $permission)
        {
            if($format == 'full')
            {
                $permissions[$permission->{$key}] = array(
                    'id'    => $permission->id,
    ...
    public function get_user_permissions($user_id = FALSE)
    {
        ...
        {
            $permission   =   ( array_key_exists($perm->system_permission_id, $permissions) ) ? $permissions[$perm->system_permission_id] : FALSE;
    ...
    public function add_permission_to_user($user_id = FALSE, $perm_id = FALSE, $value = 0)
    {
        ...
        $data = array('user_id' => $user_id, 'system_permission_id' => $perm_id);
    ...
    public function remove_permission_from_user($user_id = FALSE, $perm_id = FALSE)
    {
        ...
        if( ! $this->db->delete($this->tables['users_permissions'], array('user_id' => $user_id, 'system_permission_id' => $perm_id)) )
        {
    ...
    public function remove_permission_from_group($group_id = FALSE, $perm_id = FALSE)
    {
        ...
        $this->db->delete($this->tables['group_permissions'], array('role_id' => $group_id, 'system_permission_id' => $perm_id));
    ...
    public function get_group_permissions($group_id = FALSE)
    {
        ...
        if( is_array($group_id) )
            $this->db->where_in('role_id', $group_id);
        else
        ...
        foreach( $query->result() as $perm )
        {
            ...
            $group_permissions[$permission['key']]	=	array(
                ...
                'key'				=>	$permission['key'],
                'inherited'		    =>	TRUE,
                'active_status'		=>	($perm->active_status === '1') ? TRUE : FALSE,
            );
        }
